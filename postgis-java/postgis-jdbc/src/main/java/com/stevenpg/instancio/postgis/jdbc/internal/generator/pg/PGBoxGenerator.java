...
/** Generator for {@link PGbox}. */
public class PGBoxGenerator implements Generator<PGbox>, NumericRangeSpec<PGBoxGenerator> {
    ...
    @Override
    public PGBoxGenerator yRange(double minY, double maxY) {
        pointGen.yRange(minY, maxY);
        return this;
    }

    @Override
    public PGbox generate(Random random) {
        PGpoint a = pointGen.generate(random);
        PGpoint b = pointGen.generate(random);
        return new PGbox(a, b);
    }
}
