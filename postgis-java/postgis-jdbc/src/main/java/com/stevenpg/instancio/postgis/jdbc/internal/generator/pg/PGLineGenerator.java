...
/** Generator for {@link PGline}. */
public class PGLineGenerator implements Generator<PGline>, NumericRangeSpec<PGLineGenerator> {
    ...
    @Override
    public PGLineGenerator yRange(double minY, double maxY) {
        pointGen.yRange(minY, maxY);
        return this;
    }

    @Override
    public PGline generate(Random random) {
        PGpoint a = pointGen.generate(random);
        PGpoint b = pointGen.generate(random);
        return new PGline(a, b);
    }
}
